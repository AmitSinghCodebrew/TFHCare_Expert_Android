...
class AddMoneyActivity : DaggerAppCompatActivity(), PaymentResultListener {
    ...
    private fun bindObservers() {
        ...
        viewModel.razorPayCreateOrder.observe(this, Observer {
            it ?: return@Observer
            when (it.status) {
                Status.SUCCESS -> {
                    progressDialog.setLoading(false)
    ...
    private fun startRazorPayPayment(amount: String, orderId: String) {
        ...
        * */
        ...
        val co = Checkout()
        co.setKeyID(appClientDetails.gateway_key ?: "rzp_test_NIJ8Fwm7fvVNDU")
        ...
        try {
            ...
            options.put("image", getImageBaseUrl(true,appClientDetails.applogo ?:""))
